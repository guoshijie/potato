...
class AlipayModel extends Model{
	...
	private function updateOrder($out_trade_no,$pay_amount){
		...
		//更新订单
		DB::table('order')->where('order_no',$out_trade_no)->where('is_delete',0)->update(array('pay_status'=>2));

		$update_order_suppliers = DB::table('order_suppliers')->where('order_no',$out_trade_no)->where('is_delete',0)->update(array('pay_status'=>2));
